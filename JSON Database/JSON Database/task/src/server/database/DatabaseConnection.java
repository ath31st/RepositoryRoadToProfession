...
public class DatabaseConnection implements Connection {
    ...
    public synchronized Response get(Task task) {
        ...
        List<RecordInBase> list = operator.read();
        RecordInBase record = list.stream()
    ...
    public synchronized Response set(Task task) {
        ...
        if (record == null) {
            list.add(new RecordInBase(keys.get(0), task.getValue()));
        } else if (keys.size() >= 2) {
            JsonElement elem = record.getValue();
            ...
            list.set(index, new RecordInBase(keys.get(0), task.getValue()));
        }
        operator.save(list);
        return Response.OK;
    }
